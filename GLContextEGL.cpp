...
bool GLContextEGL::CreateContext(const Version &version, EGLContext share_context)
{
	...
	{
		printf(fmt::format("eglChooseConfig() failed: 0x{:x}", eglGetError()).c_str(), "%s \n");
		return false;
	}
...
{
	const uint32_t width = std::max<uint32_t>(m_wi.surface_width, 1);
	const uint32_t height = std::max<uint32_t>(m_wi.surface_height, 1);
	...
	if (!m_surface)
	{
		printf("eglCreatePbufferSurface() failed: 0x{:x}", eglGetError());
		return false;
	}

	printf(fmt::format("Created {}x{} pbuffer surface", width, height).c_str(), "%s");
	return true;
}
...
{
	m_wi = {};
	if (!ChangeSurface(m_wi))
		return;
}

bool GLContextEGL::UpdateWindow()
{
	DestroySurface();
...
bool GLContextEGL::CreateContextAndSurface(const Version &version, EGLContext share_context, bool make_current)
{
	...
	{
		printf(fmt::format("eglMakeCurrent() failed: 0x{:x}", eglGetError()).c_str() + '\n');
		if (m_surface != EGL_NO_SURFACE)
		{
			eglDestroySurface(m_display, m_surface);
			m_surface = EGL_NO_SURFACE;
...
EGLDisplay GLContextEGL::TryGetPlatformDisplay(EGLenum platform, const char *platform_ext)
{
	...
	if (!extensions_str)
	{
		printf("No extensions supported. \n");
	...
	{
		printf(fmt::format("Using EGL platform {}.", platform_ext).c_str());
		...
		if (get_platform_display_ext)
		{
			...
			{
				const EGLint err = eglGetError();
				printf(fmt::format("eglGetPlatformDisplayEXT() failed: {} (0x{:X})", err, err).c_str() + '\n');
			}
		}
		else
		{
			printf("eglGetPlatformDisplayEXT() was not found \n");
		}
	}
	else
	{
		printf(fmt::format("{} is not supported.", platform_ext).c_str() + '\n');
	}
...
bool GLContextEGL::Initialize(std::span<const Version> versions_to_try)
{
	...
	if (!GLAD_EGL_KHR_surfaceless_context)
	{
		printf("EGL implementation does not support surfaceless contexts, emulating with pbuffers");
	}
